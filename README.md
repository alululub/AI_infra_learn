...
| 21 | 2026-09-12 | Batched GEMM | Batched GEMM 属于水平方向的算子批处理（Batching），与垂直方向的“算子融合”有本质区别。算子融合针对存在前后依赖的上下游算子，旨在将中间数据保留在片上寄存器或共享内存中，核心价值是减少全局显存（GMEM）的访存读写；而 Batched GEMM 面向多个结构相同但数据完全独立的子矩阵。其核心优势在于：通过升维 3D Grid 一次性下发计算，消除了多次启动 Kernel 带来的驱动下发延迟；同时将海量并发 Block 均匀分发到 GPU 的所有 SM 上，消除了小矩阵无法填满算力的尾巴效应，极大提升了整体硬件利用率 |
| 22 |  |  |  |
| 23 |  |  |  |
| 24 |  |  |  |
| 25 |  |  |  |
