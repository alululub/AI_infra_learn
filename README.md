...
| 19 | 2026-08-31 | PTXmma | PTX MMA（如 mma.sync.aligned.m16n8k16）是直接映射 GPU Tensor Core 硬件乘加电路的底层汇编基元。不同于 C++ WMMA 的“黑盒容器”，PTX MMA 具备寄存器白盒透明特性，开发者可显式掌控 32 线程的物理寄存器映射（Lane Mapping）。这使得矩阵乘结果能直接保留在片上寄存器中，原地完成偏置叠加、激活函数或规约，实现零显存落盘的深度算子融合（Epilogue Fusion），是大模型高性能算子（如 FlashAttention）极致压榨硬件算力的核心支柱。 |
| 20 |  |  |  |
| 21 |  |  |  |
| 22 |  |  |  |
| 23 |  |  |  |
