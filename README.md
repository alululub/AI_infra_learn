...
| 11 | 2026-07-31 | SGEMM_share_32X32 | SGEMM_share_32X32实现的是一个基于共享内存的 SGEMM（矩阵乘法）优化版本。它通过把矩阵 A 和 B 的子块分别加载到 GPU 的共享内存中，减少对全局内存的访问次数，从而提升计算性能。每个线程块负责处理一个输出矩阵的小块，线程之间通过共享内存和同步机制协同完成乘加运算。相比朴素的矩阵乘法实现，这种方法更适合 GPU 并行计算，尤其在矩阵规模较大时，性能提升更明显。 |
| 12 |  |  |  |
| 13 |  |  |  |
| 14 |  |  |  |
| 15 |  |  |  |
